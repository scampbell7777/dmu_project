...
def train_policy(trajectories, state_dim, action_dim, epochs=100):
    states = torch.cat([torch.tensor(traj['states']) for traj in trajectories])
    actions = torch.cat([torch.tensor(traj['actions']) for traj in trajectories])
    dataset = TensorDataset(states.float(), actions.float())
    
    policy = ImitationPolicy(state_dim, action_dim).double()
    optimizer = torch.optim.Adam(policy.parameters(), lr=3e-4)
    criterion = nn.MSELoss()
    
    policy.train()
    for epoch in range(epochs):
        for batch_states, batch_actions in DataLoader(dataset, batch_size=256, shuffle=True):
            optimizer.zero_grad()
            pred_actions = policy(batch_states.double())
            loss = criterion(pred_actions, batch_actions.double())
            loss.backward()
            optimizer.step()
    policy.eval()
    return policy
...
def evaluate_policy(policy, n_episodes=20):
    ...
    for _ in range(n_episodes):
        ...
        for _ in range(1000):
            with torch.no_grad():
                state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).double()
                action = policy(state_tensor).float().squeeze(0).numpy()
...
def main():
    ...
    # Policy Imitation Section
    print("\n=== Training Imitation Policy ===")
    mpc_trajectories = collect_optimized_trajectories(
        dynamics_model,
        num_episodes=10,
        horizon=5,
        iterations=5,
    ...
    policy = train_policy(mpc_trajectories, state_dim, action_dim)
    avg_reward = evaluate_policy(policy)
    
    print(f"\nImitation Policy Average Reward: {avg_reward:.1f}")
    
    # Plotting for Imitation Learning Reward
    plt.figure(figsize=(8, 6))
    plt.bar(['Imitation Policy'], [avg_reward], color='green')
    plt.ylabel('Average Reward')
    plt.title('Imitation Policy Average Reward')
    plt.grid(axis='y')
    plt.savefig('imitation_reward.png')
    plt.show()
